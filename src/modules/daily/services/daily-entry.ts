...
// Shared by /api/v1/daily (GET, on-demand for whoever visits first each
// day) and /api/v1/push/send-daily (the cron-triggered push sender) so
// both use exactly the same "get today's real entry, generating and
// caching it once if it doesn't exist yet" logic rather than two
// implementations drifting apart.
//
// Generates real content once per calendar date via Claude, caches it in
// DailyGenerated, and serves the cached row to every subsequent caller
// that same day. Deliberately does NOT let the AI generate the verse text
// itself -- the AI companion's own system prompt forbids fabricating
// Scripture, so this asks Claude only for a reference plus
// reflection/prayer/challenge, then fetches the real verse text from our
// actual Bible API (BSB) to guarantee accuracy.
...
export async function getOrCreateDailyEntry(date: string) {
  ...
  return generateTodayEntry(date)
}
