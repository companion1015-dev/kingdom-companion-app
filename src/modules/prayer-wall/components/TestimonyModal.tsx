      ...
      <div className="fixed inset-x-3 bottom-0 sm:inset-auto sm:left-1/2 sm:-translate-x-1/2 sm:top-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-lg z-50 bg-white dark:bg-navy-dark rounded-t-3xl sm:rounded-3xl shadow-2xl shadow-navy/25 flex flex-col max-h-[92vh]"
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-navy/8 shrink-0 bg-green-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-display text-lg font-semibold text-navy dark:text-cream">Praise God — Share Your Testimony</h2>
              <p className="text-xs text-charcoal/40 dark:text-cream/40 font-body mt-0.5 truncate max-w-[280px]">{requestTitle}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl text-charcoal/35 dark:text-cream/35 hover:text-navy dark:text-cream hover:bg-navy/5 transition-colors shrink-0" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-charcoal/30 dark:text-cream/30 font-body mt-1 text-right">{testimony.length}/2000</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {PRAISE_CATEGORIES.map(cat => (
                <button key={cat.id} onClick={() => setCategory(cat.id)}
                  className={`px-3 py-2 rounded-xl border text-xs font-body transition-all ${category === cat.id ? 'bg-navy text-white border-navy' : 'border-navy/10 text-charcoal/55 dark:text-cream/55 hover:border-navy/25'}`}>
                  {cat.label}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="block text-xs font-body font-semibold text-navy/50 dark:text-cream/50 tracking-wider uppercase mb-1.5">
              Bible verse <span className="text-charcoal/30 dark:text-cream/30 font-normal normal-case">Optional</span>
            </label>
            <input value={bibleVerse} onChange={e => setBibleVerse(e.target.value)} maxLength={100}
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-navy/12 focus:border-gold/40 text-navy dark:text-cream font-body text-sm outline-none transition-all" />
          </div>

          <div>
            <label className="block text-xs font-body font-semibold text-navy/50 dark:text-cream/50 tracking-wider uppercase mb-1.5">
              Thanksgiving <span className="text-charcoal/30 dark:text-cream/30 font-normal normal-case">Optional</span>
            </label>
          ...
          <label className="flex items-center gap-2.5 cursor-pointer p-3 rounded-xl bg-cream dark:bg-navy-dark">
            <input type="checkbox" checked={isPublic} onChange={e => setIsPublic(e.target.checked)}
              className="w-4 h-4 rounded border-navy/20 text-navy dark:text-cream focus:ring-gold/30" />
            <div>
              <p className="text-sm font-body font-medium text-navy dark:text-cream">Share publicly in Praise Reports</p>
              <p className="text-xs text-charcoal/40 dark:text-cream/40 font-body">Encourage others walking through something similar</p>
            </div>
          </label>
        </div>

        <div className="px-6 py-4 border-t border-navy/8 flex gap-3 shrink-0">
          <button onClick={onClose} className="flex-1 py-3 border border-navy/15 text-navy/60 dark:text-cream/60 hover:text-navy dark:text-cream text-sm font-body font-medium rounded-2xl transition-all">
            Cancel
          </button>
