    ...
    <div className="min-h-screen bg-cream dark:bg-navy-dark-gradient">
      ...
      <main className="max-w-2xl mx-auto px-4 py-10">
        <div className="flex items-center gap-3 mb-2">
          <Mail className="w-7 h-7 text-navy dark:text-cream" />
          <h1 className="text-3xl font-serif text-navy dark:text-cream">Contact Us</h1>
        </div>
        ...
        {sent ? (
          <div className="bg-white dark:bg-navy-dark rounded-2xl border border-navy/8 p-8 text-center">
            ...
            <h2 className="font-display text-xl font-semibold text-navy dark:text-cream mb-2">Message sent</h2>
            <p className="text-charcoal/55 dark:text-cream/55 font-body text-sm">
              Thank you for reaching out — we read every message and will get back to you where a reply is needed.
            </p>
          </div>
        ) : (
          <div className="bg-white dark:bg-navy-dark rounded-2xl border border-navy/8 p-6 sm:p-8 space-y-4">
            {error && <p className="text-xs font-body text-red-600 bg-red-50 border border-red-100 rounded-xl p-3">{error}</p>}

            <div>
              <label className="block text-xs font-body font-semibold text-navy/50 dark:text-cream/50 tracking-wider uppercase mb-1.5">Type</label>
              <div className="flex gap-2">
                {TYPES.map(t => (
                  <button key={t} onClick={() => setType(t)}
                    className={`px-3 py-1.5 rounded-full border text-xs font-body font-medium transition-all ${type === t ? 'bg-navy text-white border-navy' : 'border-navy/12 text-charcoal/55 dark:text-cream/55 hover:border-navy/30'}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-xs font-body font-semibold text-navy/50 dark:text-cream/50 tracking-wider uppercase mb-1.5">Subject</label>
            ...
            <div>
              <label className="block text-xs font-body font-semibold text-navy/50 dark:text-cream/50 tracking-wider uppercase mb-1.5">Message</label>
              ...
              <p className="text-xs text-charcoal/30 dark:text-cream/30 font-body mt-1 text-right">{message.length}/2000</p>
            </div>
