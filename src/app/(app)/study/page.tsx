...
export default function StudyPage() {
  ...
  const empty = (label: string, Icon: typeof Bookmark, hint: string) => (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <Icon className="w-12 h-12 text-navy/15 dark:text-cream/15 mb-4" />
      <p className="font-body text-sm text-charcoal/50 dark:text-cream/50 mb-1">No {label} yet</p>
      <p className="font-body text-xs text-charcoal/35 dark:text-cream/35 mb-6">{hint}</p>
      <Link
        href="/bible"
    ...
    <>
      ...
      <main className="min-h-screen bg-cream dark:bg-navy-dark pt-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
          ...
          {/* Header */}
          <div className="mb-8">
            <h1 className="font-display text-3xl font-light text-navy dark:text-cream mb-1">Study Workspace</h1>
          ...
          <div className="relative mb-6">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-charcoal/35 dark:text-cream/35 pointer-events-none" />
            <input
              type="search"
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search your study notes…"
              className="w-full pl-11 pr-4 py-3 bg-white dark:bg-navy-dark rounded-xl border border-navy/10 focus:border-gold/40 focus:ring-2 focus:ring-gold/10 text-navy dark:text-cream font-body text-sm outline-none transition-all"
            />
          </div>

          {/* Tabs */}
          <div className="flex gap-2 mb-6 bg-white dark:bg-navy-dark rounded-xl p-1.5 border border-navy/8">
              ...
              <button
                ...
                className={`
                  ...
                  ${tab === id
                    ...
                    : 'text-charcoal/50 dark:text-cream/50 hover:text-navy dark:text-cream hover:bg-navy/5'
                  }
                `}
              >
                ...
                {count > 0 && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${tab === id ? 'bg-white/20 dark:bg-navy-dark text-white' : 'bg-navy/8 text-navy/50 dark:text-cream/50'}`}>
                    {count}
                  </span>
                )}
              </button>
            ...
            <div className="space-y-2">
              {bookmarkList.length === 0
                ...
                : bookmarkList.map(([verseId, data]) => (
                  <div
                    key={verseId}
                    className="flex items-center gap-4 p-4 bg-white dark:bg-navy-dark rounded-xl border border-navy/8 hover:border-gold/20 group transition-all"
                  >
                    <Bookmark className="w-4 h-4 text-gold shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-body font-semibold text-navy dark:text-cream">{data.reference}</p>
                      {data.collection && (
                        <p className="text-xs text-charcoal/40 dark:text-cream/40 font-body mt-0.5">{data.collection}</p>
                      )}
                    </div>
                    <Link
                      href={`/bible`}
                    ...
                    <button
                      ...
                      className="p-2 text-charcoal/20 dark:text-cream/20 hover:text-red-400 transition-colors"
            ...
            <div className="space-y-3">
              {noteList.length === 0
                ...
                : noteList.map(([verseId, data]) => (
                  <div
                    key={verseId}
                    className="p-5 bg-white dark:bg-navy-dark rounded-xl border border-navy/8 hover:border-gold/20 group transition-all"
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 text-charcoal/20 dark:text-cream/20 hover:text-red-400 transition-colors rounded-lg hover:bg-red-50"
                    ...
                    <p className="text-sm font-body text-charcoal/70 dark:text-cream/70 leading-relaxed line-clamp-3">
                      {data.content}
                    </p>
                    {data.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {data.tags.map(tag => (
                          <span key={tag} className="px-2 py-0.5 rounded-full bg-navy/6 text-navy/50 dark:text-cream/50 text-xs font-body">
                            #{tag}
                          </span>
                        ))}
                      </div>
          ...
          {!authed && (
            <div className="mt-10 p-6 rounded-2xl bg-navy/4 border border-navy/8 text-center">
              <p className="font-display text-base text-navy dark:text-cream mb-1">
                Create a free account to sync across devices
              </p>
              <p className="text-xs text-charcoal/45 dark:text-cream/45 font-body mb-4">
