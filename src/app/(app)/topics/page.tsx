    ...
    <div className="min-h-screen bg-cream dark:bg-navy-dark-gradient">
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        <div className="flex items-center gap-3 mb-2">
          <Compass className="w-7 h-7 text-navy dark:text-cream" />
          <h1 className="text-2xl font-serif text-navy dark:text-cream">Topics</h1>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {topics.map(t => (
              <Link
                key={t.slug}
                href={`/topics/${t.slug}`}
                className="p-4 rounded-2xl bg-white dark:bg-navy-dark hover:bg-cream dark:bg-navy-dark border border-navy/8 hover:border-gold/20 text-center transition-all duration-200 hover:shadow-md hover:-translate-y-0.5"
              >
                <span className="font-body text-sm font-medium text-navy/80 dark:text-cream/80">{t.name}</span>
              </Link>
            ))}
          </div>
        )}
